...
export const createCourse = async (req, res) => {
  ...
  try {
    ...
    if (exists) {
      res
        .status(424)
        .json({ error: "A course with that title already exists" });
      return;
    }
...
export const getQuestions = async (req, res) => {
  try{
    ...
    const course = await Course.find({title: title, category: category[0].toUpperCase() + category.slice(1).toLowerCase(j)});
...
export const getCourses = async (req, res) => {
  await Course.find({
    category: { $regex: new RegExp("^" + req.params.courseCat + "$", "i") },
  })
    .then((c) => {
